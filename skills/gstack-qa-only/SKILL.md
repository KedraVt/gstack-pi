...
"test but don't fix". For the full test-fix-verify loop, use /skill:gstack-qa instead.
...
C:/Users/Mattia/.pi/agent/extensions/gstack-pi/runtime/bin/gstack-timeline-log '{"skill":"qa-only","event":"started","branch":"'"$_BRANCH"'","session":"'"$_SESSION_ID"'"}' 2>/dev/null &
_HAS_ROUTING="no"
...
# /skill:gstack-qa-only: Report-Only QA Testing
...
1. **Project-scoped test plans:** Check `~/.gstack/projects/` for recent `*-test-plan-*.md` files for this repo
   ...
   ```
2. **Conversation context:** Check if a prior `/skill:gstack-plan-eng-review` or `/skill:gstack-plan-ceo-review` produced test plan output in this conversation
...
This is the **primary mode** for developers verifying their work. When the user says `/skill:gstack-qa` without a URL and the repo is on a feature branch, automatically:
...
2. **Identify affected pages/routes** from the changed files:
   ...
   **If no obvious pages/routes are identified from the diff:** Do not skip browser testing. The user invoked /skill:gstack-qa because they want browser-based verification. Fall back to Quick mode — navigate to the homepage, follow the top 5 navigation targets, check console for errors, and test any interactive elements found. Backend, config, and infrastructure changes affect app behavior — always verify the app still works.
...
3. Copy report template from `C:/Users/Mattia/.pi/agent/extensions/gstack-pi/source/qa/templates/qa-report-template.md` to output dir
...
Then follow the **per-page exploration checklist** (see `C:/Users/Mattia/.pi/agent/extensions/gstack-pi/source/qa/references/issue-taxonomy.md`):
...
**Write each issue to the report immediately** using the template format from `C:/Users/Mattia/.pi/agent/extensions/gstack-pi/source/qa/templates/qa-report-template.md`.
...
```bash
C:/Users/Mattia/.pi/agent/extensions/gstack-pi/source/bin/gstack-learnings-log '{"skill":"qa-only","type":"TYPE","key":"SHORT_KEY","insight":"DESCRIPTION","confidence":N,"source":"SOURCE","files":["path/to/relevant/file"]}'
```
...
11. **Never fix bugs.** Find and document only. Do not read source code, edit files, or suggest fixes in the report. Your job is to report what's broken, not to fix it. Use `/skill:gstack-qa` for the test-fix-verify loop.
12. **No test framework detected?** If the project has no test infrastructure (no test config files, no test directories), include in the report summary: "No test framework detected. Run `/skill:gstack-qa` to bootstrap one and enable regression test generation."
