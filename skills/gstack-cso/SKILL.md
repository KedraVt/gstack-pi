...
C:/Users/Mattia/.pi/agent/extensions/gstack-pi/runtime/bin/gstack-timeline-log '{"skill":"cso","event":"started","branch":"'"$_BRANCH"'","session":"'"$_SESSION_ID"'"}' 2>/dev/null &
_HAS_ROUTING="no"
...
# /skill:gstack-cso — Chief Security Officer Audit (v2)
...
## User-invocable
When the user types `/skill:gstack-cso`, run this skill.

## Arguments
- `/skill:gstack-cso` — full daily audit (all phases, 8/10 confidence gate)
- `/skill:gstack-cso --comprehensive` — monthly deep scan (all phases, 2/10 bar — surfaces more)
- `/skill:gstack-cso --infra` — infrastructure-only (Phases 0-6, 12-14)
- `/skill:gstack-cso --code` — code-only (Phases 0-1, 7, 9-11, 12-14)
- `/skill:gstack-cso --skills` — skill supply chain only (Phases 0, 8, 12-14)
- `/skill:gstack-cso --diff` — branch changes only (combinable with any above)
- `/skill:gstack-cso --supply-chain` — dependency audit only (Phases 0, 3, 12-14)
- `/skill:gstack-cso --owasp` — OWASP Top 10 only (Phases 0, 9, 12-14)
- `/skill:gstack-cso --scope auth` — focused audit on a specific domain
...
> **STOP.** Before running the scope-dependent audit phases (Phases 2-11) selected by the resolved mode, after the Phase 0 stack detection and Phase 1 attack-surface census, Read `C:/Users/Mattia/.pi/agent/extensions/gstack-pi/source/cso/sections/audit-phases.md` and execute it
...
**Daily mode (default, `/skill:gstack-cso`):** 8/10 confidence gate. Zero noise. Only report what you're sure about.
...
**Comprehensive mode (`/skill:gstack-cso --comprehensive`):** 2/10 confidence gate. Filter true noise only (test fixtures, documentation, placeholders) but include anything that MIGHT be a real issue. Flag these as `TENTATIVE` to distinguish from confirmed findings.
...
```bash
C:/Users/Mattia/.pi/agent/extensions/gstack-pi/source/bin/gstack-learnings-log '{"skill":"cso","type":"TYPE","key":"SHORT_KEY","insight":"DESCRIPTION","confidence":N,"source":"SOURCE","files":["path/to/relevant/file"]}'
```
...
**This tool is not a substitute for a professional security audit.** /skill:gstack-cso is an AI-assisted
...
sensitive data, payments, or PII, engage a professional penetration testing firm. Use /skill:gstack-cso as
...
**Always include this disclaimer at the end of every /skill:gstack-cso report output.**
