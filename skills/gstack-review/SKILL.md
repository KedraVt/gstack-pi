...
C:/Users/Mattia/.pi/agent/extensions/gstack-pi/runtime/bin/gstack-timeline-log '{"skill":"review","event":"started","branch":"'"$_BRANCH"'","session":"'"$_SESSION_ID"'"}' 2>/dev/null &
_HAS_ROUTING="no"
...
You are running the `/skill:gstack-review` workflow. Analyze the current branch's diff against the base branch for structural issues that tests don't catch.
...
1. Read `TODOS.md` (if it exists). Read PR description (`gh pr view --json body --jq .body 2>/dev/null || true`).
   ...
   **If no PR exists:** rely on commit messages and TODOS.md for stated intent — this is the common case since /skill:gstack-review runs before /skill:gstack-ship creates the PR.
...
```bash
C:/Users/Mattia/.pi/agent/extensions/gstack-pi/source/bin/gstack-learnings-log '{
  "type": "pitfall",
...
Read `C:/Users/Mattia/.pi/agent/extensions/gstack-pi/source/review/checklist.md`.
...
Read `C:/Users/Mattia/.pi/agent/extensions/gstack-pi/source/review/greptile-triage.md` and follow the fetch, filter, classify, and **escalation detection** steps.
...
- Otherwise, include ONE line in the review output: `Version claimed: v<BRANCH_VERSION>. Queue: <CLAIMED_COUNT> PR(s) ahead. <VERDICT>` where VERDICT is either `Slot free` (if `BRANCH_VERSION >= NEXT_SLOT`) or `⚠ queue moved — rerun /skill:gstack-ship to reconcile v<BRANCH_VERSION> → v<NEXT_SLOT>`.
...
```bash
_CROSS_PROJ=$(C:/Users/Mattia/.pi/agent/extensions/gstack-pi/runtime/bin/gstack-config get cross_project_learnings 2>/dev/null || echo "unset")
echo "CROSS_PROJECT: $_CROSS_PROJ"
if [ "$_CROSS_PROJ" = "true" ]; then
  C:/Users/Mattia/.pi/agent/extensions/gstack-pi/runtime/bin/gstack-learnings-search --limit 10 --cross-project 2>/dev/null || true
else
  C:/Users/Mattia/.pi/agent/extensions/gstack-pi/runtime/bin/gstack-learnings-search --limit 10 2>/dev/null || true
fi
```
...
If A: run `C:/Users/Mattia/.pi/agent/extensions/gstack-pi/runtime/bin/gstack-config set cross_project_learnings true`
If B: run `C:/Users/Mattia/.pi/agent/extensions/gstack-pi/runtime/bin/gstack-config set cross_project_learnings false`
...
```bash
source <(C:/Users/Mattia/.pi/agent/extensions/gstack-pi/runtime/bin/gstack-diff-scope <base> 2>/dev/null) || true
...
```bash
C:/Users/Mattia/.pi/agent/extensions/gstack-pi/runtime/bin/gstack-specialist-stats 2>/dev/null || true
```
...
1. **Testing** — read `C:/Users/Mattia/.pi/agent/extensions/gstack-pi/source/review/specialists/testing.md`
2. **Maintainability** — read `C:/Users/Mattia/.pi/agent/extensions/gstack-pi/source/review/specialists/maintainability.md`
...
3. **Security** — if SCOPE_AUTH=true, OR if SCOPE_BACKEND=true AND DIFF_LINES > 100. Read `C:/Users/Mattia/.pi/agent/extensions/gstack-pi/source/review/specialists/security.md`
4. **Performance** — if SCOPE_BACKEND=true OR SCOPE_FRONTEND=true. Read `C:/Users/Mattia/.pi/agent/extensions/gstack-pi/source/review/specialists/performance.md`
5. **Data Migration** — if SCOPE_MIGRATIONS=true. Read `C:/Users/Mattia/.pi/agent/extensions/gstack-pi/source/review/specialists/data-migration.md`
6. **API Contract** — if SCOPE_API=true. Read `C:/Users/Mattia/.pi/agent/extensions/gstack-pi/source/review/specialists/api-contract.md`
7. **Design** — if SCOPE_FRONTEND=true. Use the existing design review checklist at `C:/Users/Mattia/.pi/agent/extensions/gstack-pi/source/review/design-checklist.md`
...
```bash
C:/Users/Mattia/.pi/agent/extensions/gstack-pi/runtime/bin/gstack-learnings-search --type pitfall --query "{specialist domain}" --limit 5 2>/dev/null || true
```
...
1. The red-team checklist from `C:/Users/Mattia/.pi/agent/extensions/gstack-pi/source/review/specialists/red-team.md`
...
2. If the doc file was NOT updated in this branch but the code it describes WAS changed, flag it as an INFORMATIONAL finding:
   "Documentation may be stale: [file] describes [feature/component] but code changed in this branch. Consider running `/skill:gstack-document-release`."

This is informational only — never critical. The fix action is `/skill:gstack-document-release`.
...
_TEL=$(C:/Users/Mattia/.pi/agent/extensions/gstack-pi/runtime/bin/gstack-config get telemetry 2>/dev/null || echo off)
_CODEX_CFG=$(C:/Users/Mattia/.pi/agent/extensions/gstack-pi/runtime/bin/gstack-config get codex_reviews 2>/dev/null || echo enabled)
source C:/Users/Mattia/.pi/agent/extensions/gstack-pi/runtime/bin/gstack-codex-probe 2>/dev/null || true
...
```bash
C:/Users/Mattia/.pi/agent/extensions/gstack-pi/runtime/bin/gstack-review-log '{"skill":"adversarial-review","timestamp":"'"$(date -u +%Y-%m-%dT%H:%M:%SZ)"'","status":"STATUS","source":"SOURCE","tier":"always","gate":"GATE","commit":"'"$(git rev-parse --short HEAD)"'"}'
```
...
After all review passes complete, persist the final `/skill:gstack-review` outcome so `/skill:gstack-ship` can
recognize that Eng Review was run on this branch.

Run:

```bash
C:/Users/Mattia/.pi/agent/extensions/gstack-pi/runtime/bin/gstack-review-log '{"skill":"review","timestamp":"TIMESTAMP","status":"STATUS","issues_found":N,"critical":N,"informational":N,"quality_score":SCORE,"specialists":SPECIALISTS_JSON,"findings":FINDINGS_JSON,"commit":"COMMIT"}'
```
...
```bash
C:/Users/Mattia/.pi/agent/extensions/gstack-pi/source/bin/gstack-learnings-log '{"skill":"review","type":"TYPE","key":"SHORT_KEY","insight":"DESCRIPTION","confidence":N,"source":"SOURCE","files":["path/to/relevant/file"]}'
```
...
- **Fix-first, not read-only.** AUTO-FIX items are applied directly. ASK items are only applied after user approval. Never commit, push, or create PRs — that's /skill:gstack-ship's job.
