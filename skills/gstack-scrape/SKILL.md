...
C:/Users/Mattia/.pi/agent/gstack-pi/repo/bin/gstack-timeline-log '{"skill":"scrape","event":"started","branch":"'"$_BRANCH"'","session":"'"$_SESSION_ID"'"}' 2>/dev/null &
_HAS_ROUTING="no"
...
```bash
C:/Users/Mattia/.pi/agent/gstack-pi/repo/bin/gstack-question-log '{"skill":"scrape","question_id":"<id>","question_summary":"<short>","category":"<approval|clarification|routing|cherry-pick|feedback-loop>","door_type":"<one-way|two-way>","options_count":N,"user_choice":"<key>","recommended":"<key>","session_id":"'"$_SESSION_ID"'"}' 2>/dev/null || true
```
...
# /skill:gstack-scrape — pull data from a page
...
2. **Prototype path** (~30s) — no matching skill yet, so drive the page
   with `$B` primitives, return the JSON, and suggest `/skill:gstack-skillify` so the
...
The user's request after `/skill:gstack-scrape` is the intent. If they did not include
one, ask once:
...
> "/skill:gstack-scrape is read-only. For mutating flows, use /automate (browser-skills
...
> "Say /skill:gstack-skillify to make this a permanent skill (200ms on next call)."
...
- Do NOT suggest /skill:gstack-skillify on a broken prototype.
...
- Auth flows / cookie import (use /skill:gstack-setup-browser-cookies first)
...
- Do not embed prose around the JSON in the chat reply unless the user
  asked for an explanation — many `/skill:gstack-scrape` callers pipe the output to
  `jq`.
...
```bash
C:/Users/Mattia/.pi/agent/gstack-pi/repo/bin/gstack-learnings-log '{"skill":"scrape","type":"TYPE","key":"SHORT_KEY","insight":"DESCRIPTION","confidence":N,"source":"SOURCE","files":["path/to/relevant/file"]}'
```
