...
Files the issue,
optionally spawns a Claude Code agent in a fresh worktree, and lets /skill:gstack-ship close
...
C:/Users/Mattia/.pi/agent/extensions/gstack-pi/runtime/bin/gstack-timeline-log '{"skill":"spec","event":"started","branch":"'"$_BRANCH"'","session":"'"$_SESSION_ID"'"}' 2>/dev/null &
_HAS_ROUTING="no"
...
# /skill:gstack-spec — Author a Backlog-Ready Spec (issue + optional agent spawn)
...
When the user invokes `/skill:gstack-spec`, scan their message for these flags. Flags are space-
...
- **`gh` not authenticated:** print: "Dedupe skipped — `gh auth status` reports
  not logged in. Run `gh auth login` and re-invoke `/skill:gstack-spec` to enable duplicate
...
bun C:/Users/Mattia/.pi/agent/extensions/gstack-pi/source/lib/redact-audit-log.ts \
...
taxonomy: `lib/redact-patterns.ts` or `/skill:gstack-cso`. Run it on the EXACT spec bytes
...
REDACT_VIS=$(C:/Users/Mattia/.pi/agent/extensions/gstack-pi/runtime/bin/gstack-config get redact_repo_visibility 2>/dev/null)
...
REDACT_BODY_EOF
REDACT_JSON=$(C:/Users/Mattia/.pi/agent/extensions/gstack-pi/source/bin/gstack-redact --from-file "$REDACT_FILE" --repo-visibility "$REDACT_VIS" --self-email "$(git config user.email 2>/dev/null)" --json)
REDACT_CODE=$?
```
...
- **codex not authenticated** (stderr contains "auth"/"login"/"unauthorized"):
  ...
  re-invoke `/skill:gstack-spec`. Continuing to Phase 5." Skip.
- **Timeout (>2 min):** print: "Quality gate skipped — codex didn't respond in
  2 minutes. Skipping ensures `/skill:gstack-spec` stays usable. Run `codex doctor` to
...
reuse it; write the exact bytes to `$REDACT_FILE`; `C:/Users/Mattia/.pi/agent/extensions/gstack-pi/source/bin/gstack-redact --from-file "$REDACT_FILE"
...
C:/Users/Mattia/.pi/agent/extensions/gstack-pi/source/bin/gstack-decision-log '{"decision":"Spec filed #ISSUE_NUMBER: TITLE","rationale":"APPROACH","scope":"issue","issue":"ISSUE_NUMBER","source":"skill","confidence":7}' 2>/dev/null || true
```

The last line records the spec as a durable, issue-scoped cross-session decision so a future session (or `/skill:gstack-ship` closing the issue) inherits the core approach and why, not just the issue link. Non-interactive, best-effort (`|| true`). Substitute `ISSUE_NUMBER` (from the filed issue), `TITLE` (the issue title), and `APPROACH` (the one core approach/decision the spec settled). Only fires when the issue was actually filed.
...
is consumed by `/skill:gstack-ship` for auto-close.
...
```bash
eval "$(C:/Users/Mattia/.pi/agent/extensions/gstack-pi/runtime/bin/gstack-paths)"
eval "$(C:/Users/Mattia/.pi/agent/extensions/gstack-pi/runtime/bin/gstack-slug)"
...
The PID suffix and atomic rename prevent collisions when two `/skill:gstack-spec` invocations
...
Capture timestamps at three checkpoints, write to telemetry envelope at /skill:gstack-spec
exit:
...
`/skill:gstack-retro` is a separate follow-up.
...
- **Before `/skill:gstack-spec`:** if the user is still exploring whether to build something,
  route them to `/skill:gstack-office-hours` first. `/skill:gstack-spec` is for work that has already
  passed the "is this worth building" bar.
- **After `/skill:gstack-spec`:** if the spec describes architectural or design risk that
  needs review before implementation starts, suggest `/skill:gstack-plan-eng-review` (or
  `/skill:gstack-autoplan` for the full review gauntlet).
- **For implementation:** the issue itself is the handoff. The implementer can
  open it and execute without re-asking the user.
- **`/skill:gstack-ship` integration:** when `/skill:gstack-ship` opens a PR for a worktree that contains
  a `/skill:gstack-spec` archive (frontmatter `spec_issue_number: <N>`) AND the PR delivers
  the full spec (acceptance criteria checked off per `/skill:gstack-ship`'s existing
  plan-completion gate), `/skill:gstack-ship` adds `Closes #<N>` to the PR body so merging
