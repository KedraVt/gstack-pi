...
C:/Users/Mattia/.pi/agent/extensions/gstack-pi/runtime/bin/gstack-timeline-log '{"skill":"plan-ceo-review","event":"started","branch":"'"$_BRANCH"'","session":"'"$_SESSION_ID"'"}' 2>/dev/null &
_HAS_ROUTING="no"
...
```
If a design doc exists (from `/skill:gstack-office-hours`), read it. Use it as the source of truth for the problem statement, constraints, and chosen approach. If it has a `Supersedes:` field, note that this is a revised design.
...
from a prior CEO review session that paused so the user could run `/skill:gstack-office-hours`. Use it
...
sure," or is clearly exploring rather than reviewing — offer `/skill:gstack-office-hours`:

> "It sounds like you're still figuring out what to build — that's totally fine, but
> that's what /skill:gstack-office-hours is designed for. Want to run /skill:gstack-office-hours right now?
> We'll pick up right where we left off."

Options: A) Yes, run /skill:gstack-office-hours now. B) No, keep going.
...
eval "$(C:/Users/Mattia/.pi/agent/extensions/gstack-pi/runtime/bin/gstack-slug 2>/dev/null)" 2>/dev/null || true
{
  ...
  printf '\n### %s\n\n' "product"
  C:/Users/Mattia/.pi/agent/extensions/gstack-pi/source/bin/gstack-brain-cache get product --project "$SLUG" 2>/dev/null || printf '_(no product digest available yet)_\n'
  printf '\n### %s\n\n' "goals"
  C:/Users/Mattia/.pi/agent/extensions/gstack-pi/source/bin/gstack-brain-cache get goals --project "$SLUG" 2>/dev/null || printf '_(no goals digest available yet)_\n'
  printf '\n### %s\n\n' "recent-decisions"
  C:/Users/Mattia/.pi/agent/extensions/gstack-pi/source/bin/gstack-brain-cache get recent-decisions --project "$SLUG" 2>/dev/null || printf '_(no recent-decisions digest available yet)_\n'
  printf '\n### %s\n\n' "user-profile"
  C:/Users/Mattia/.pi/agent/extensions/gstack-pi/source/bin/gstack-brain-cache get user-profile  2>/dev/null || printf '_(no user-profile digest available yet)_\n'
...
```bash
eval "$(C:/Users/Mattia/.pi/agent/extensions/gstack-pi/runtime/bin/gstack-slug 2>/dev/null)" && mkdir -p ~/.gstack/projects/$SLUG/ceo-plans
```
...
Generated by /skill:gstack-plan-ceo-review on {date}
...
> **STOP.** Before running the 11-section deep review, required outputs, and review report (only after Step 0 scope and mode are agreed), Read `C:/Users/Mattia/.pi/agent/extensions/gstack-pi/source/plan-ceo-review/sections/review-sections.md` and execute it
...
5. If a plan file is in context for this skill invocation: confirm
   ...
   once. If no plan file is in context (e.g. `/skill:gstack-codex consult` against a
