...
metadata:
  ...
  source: document-generate/SKILL.md
---
# Pi adapter note
...
/skill:gstack-document-release when it finds coverage gaps. Use when asked to "write docs",
...
C:/Users/Mattia/.pi/agent/extensions/gstack-pi/runtime/bin/gstack-timeline-log '{"skill":"document-generate","event":"started","branch":"'"$_BRANCH"'","session":"'"$_SESSION_ID"'"}' 2>/dev/null &
_HAS_ROUTING="no"
...
You are running the `/skill:gstack-document-generate` workflow. Your job: produce **high-quality,
...
2. **From /skill:gstack-document-release** — the coverage map identified gaps; you fill them
...
1. Determine what to document:
   ...
   - **If called from /skill:gstack-document-release with gaps**: scope is the specific entities from the coverage map
...
git diff --cached --no-color | grep '^+' | sed 's/^+//' | \
  C:/Users/Mattia/.pi/agent/extensions/gstack-pi/source/bin/gstack-redact --repo-visibility "${REDACT_VIS:-unknown}" --json
