...
"save my work". Pair with /skill:gstack-context-restore to resume later.
...
C:/Users/Mattia/.pi/agent/gstack-pi/repo/bin/gstack-timeline-log '{"skill":"context-save","event":"started","branch":"'"$_BRANCH"'","session":"'"$_SESSION_ID"'"}' 2>/dev/null &
_HAS_ROUTING="no"
...
```bash
C:/Users/Mattia/.pi/agent/gstack-pi/repo/bin/gstack-question-log '{"skill":"context-save","question_id":"<id>","question_summary":"<short>","category":"<approval|clarification|routing|cherry-pick|feedback-loop>","door_type":"<one-way|two-way>","options_count":N,"user_choice":"<key>","recommended":"<key>","session_id":"'"$_SESSION_ID"'"}' 2>/dev/null || true
```
...
# /skill:gstack-context-save — Save Working Context
...
can resume without losing a beat via `/skill:gstack-context-restore`.
...
- `/skill:gstack-context-save` or `/skill:gstack-context-save <title>` → **Save**
- `/skill:gstack-context-save list` → **List**

If the user provides a title after the command (e.g., `/skill:gstack-context-save auth refactor`),
use it as the title. Otherwise, infer a title from the current work.

If the user types `/skill:gstack-context-save resume` or `/skill:gstack-context-save restore`, tell them:
"Use `/skill:gstack-context-restore` instead — save and restore are separate skills now."
...
Restore later with /skill:gstack-context-restore.
```
...
If the user passes `--all` (e.g., `/skill:gstack-context-save list --all`), show contexts
from **all branches**.
...
`/skill:gstack-context-save` to save your current working state."
...
- **This is a gstack skill, not a Claude Code built-in.** When the user types
  `/skill:gstack-context-save`, invoke this skill via the Pi skill command (`/skill:name`). The old `/checkpoint`
