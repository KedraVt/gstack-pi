...
C:/Users/Mattia/.pi/agent/extensions/gstack-pi/runtime/bin/gstack-timeline-log '{"skill":"retro","event":"started","branch":"'"$_BRANCH"'","session":"'"$_SESSION_ID"'"}' 2>/dev/null &
_HAS_ROUTING="no"
...
# /skill:gstack-retro — Weekly Engineering Retrospective

Generates a comprehensive engineering retrospective analyzing commit history, work patterns, and code quality metrics. Team-aware: identifies the user running the command, then analyzes every contributor with per-person praise and growth opportunities. Designed for a senior IC/CTO-level builder using Claude Code as a force multiplier.

## User-invocable
When the user types `/skill:gstack-retro`, run this skill.

## Arguments
- `/skill:gstack-retro` — default: last 7 days
- `/skill:gstack-retro 24h` — last 24 hours
- `/skill:gstack-retro 14d` — last 14 days
- `/skill:gstack-retro 30d` — last 30 days
- `/skill:gstack-retro compare` — compare current window vs prior same-length window
- `/skill:gstack-retro compare 14d` — compare with explicit window
- `/skill:gstack-retro global` — cross-project retro across all AI coding tools (7d default)
- `/skill:gstack-retro global 14d` — cross-project retro with explicit window
...
```
Usage: /skill:gstack-retro [window | compare | global]
  /skill:gstack-retro              — last 7 days (default)
  /skill:gstack-retro 24h          — last 24 hours
  /skill:gstack-retro 14d          — last 14 days
  /skill:gstack-retro 30d          — last 30 days
  /skill:gstack-retro compare      — compare this period vs prior period
  /skill:gstack-retro compare 14d  — compare with explicit window
  /skill:gstack-retro global       — cross-project retro across all AI tools (7d default)
  /skill:gstack-retro global 14d   — cross-project retro with explicit window
```
...
```
  EUREKA /skill:gstack-office-hours (branch: garrytan/auth-rethink): "Session tokens don't need server storage — browser crypto API makes client-side JWT validation viable"
  EUREKA /skill:gstack-plan-eng-review (branch: garrytan/cache-layer): "Redis isn't needed here — Bun's built-in LRU cache handles this workload"
```
...
```bash
C:/Users/Mattia/.pi/agent/extensions/gstack-pi/source/bin/gstack-learnings-log '{"skill":"retro","type":"TYPE","key":"SHORT_KEY","insight":"DESCRIPTION","confidence":N,"source":"SOURCE","files":["path/to/relevant/file"]}'
```
...
### Plan Completion
Check review JSONL logs for plan completion data from /skill:gstack-ship runs this period:
...
When the user runs `/skill:gstack-retro global` (or `/skill:gstack-retro global 14d`), follow this flow instead of the repo-scoped Steps 1-14. This mode works from any directory — it does NOT require being inside a git repo.
...
```bash
DISCOVER_BIN=""
[ -x C:/Users/Mattia/.pi/agent/extensions/gstack-pi/runtime/bin/gstack-global-discover ] && DISCOVER_BIN=C:/Users/Mattia/.pi/agent/extensions/gstack-pi/runtime/bin/gstack-global-discover
[ -z "$DISCOVER_BIN" ] && [ -x C:/Users/Mattia/.pi/agent/extensions/gstack-pi/runtime/bin/gstack-global-discover ] && DISCOVER_BIN=C:/Users/Mattia/.pi/agent/extensions/gstack-pi/runtime/bin/gstack-global-discover
...
If `total_sessions` is 0, say: "No AI coding sessions found in the last <window>. Try a longer window: `/skill:gstack-retro global 30d`" and stop.
...
- Top Work: 3 bullet points summarizing the user's major themes, inferred from
  ...
  E.g., "Built /skill:gstack-retro global — cross-project retrospective with AI session discovery"
  not "feat: gstack-global-discover" + "feat: /skill:gstack-retro global template".
...
When the user runs `/skill:gstack-retro compare` (or `/skill:gstack-retro compare 14d`):
