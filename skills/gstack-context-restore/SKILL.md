...
description: |
  Restore working context saved earlier by /skill:gstack-context-save. (gstack)
license: MIT
metadata:
  upstream: https://github.com/garrytan/gstack
  source: context-restore/SKILL.md
---
# Pi adapter note
...
"pick up where I left off". Pair with /skill:gstack-context-save.
...
C:/Users/Mattia/.pi/agent/extensions/gstack-pi/runtime/bin/gstack-timeline-log '{"skill":"context-restore","event":"started","branch":"'"$_BRANCH"'","session":"'"$_SESSION_ID"'"}' 2>/dev/null &
_HAS_ROUTING="no"
...
# /skill:gstack-context-restore — Restore Saved Working Context
...
intentionally different from `/skill:gstack-context-save list`, which defaults to the current
branch. `/skill:gstack-context-restore` is for Conductor workspace handoff — a context saved
on one branch can be resumed from another.

**Do NOT filter the candidate set by current branch.** The `list` flow does
that; `/skill:gstack-context-restore` does not.
...
- `/skill:gstack-context-restore` → load the most recent saved context (any branch)
- `/skill:gstack-context-restore <title-fragment-or-number>` → load a specific saved context
- `/skill:gstack-context-restore list` → tell the user "Use `/skill:gstack-context-save list` — listing
...
if [ ! -d "$CHECKPOINT_DIR" ]; then
  ...
  # context window just listing them. /skill:gstack-context-save list handles pagination.
...
"No saved contexts yet. Run `/skill:gstack-context-save` first to save your current working
state, then `/skill:gstack-context-restore` will find it."
...
- **This is a gstack skill, not a Claude Code built-in.** When the user types
  `/skill:gstack-context-restore`, invoke this skill via the Pi skill command (`/skill:name`).
