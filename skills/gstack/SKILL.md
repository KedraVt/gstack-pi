...
and dogfooding it points you at /skill:gstack-browse. Use when you invoke gstack without a specific
...
1. If the request is about a browser, QA, dogfooding, screenshots, or inspecting a page
   (open a site, test a deploy, take a screenshot, check a flow visually) → invoke `/skill:gstack-browse`.
...
If `PROACTIVE` is `true` (default): **invoke the Pi skill command (`/skill:name`)** when the user's request
matches a skill's purpose. Do NOT answer directly when a skill exists for the task.
Use the Pi skill command (`/skill:name`) to invoke it. The skill has specialized workflows, checklists, and
quality gates that produce better results than answering inline.

**Routing rules — when you see these patterns, INVOKE the skill via the Pi skill command (`/skill:name`):**
- User describes a new idea, asks "is this worth building", brainstorms, pitches a concept → invoke `/skill:gstack-office-hours`
- User asks to spec something out, file an issue, write up a ticket, "turn this into a GitHub issue", "backlog item" → invoke `/skill:gstack-spec`
- User asks about strategy, scope, ambition, "think bigger", "what should we build" → invoke `/skill:gstack-plan-ceo-review`
- User asks to review architecture, lock in the plan, "does this design make sense" → invoke `/skill:gstack-plan-eng-review`
- User asks about design system, brand, visual identity, "how should this look" → invoke `/skill:gstack-design-consultation`
- User asks to review design of a plan → invoke `/skill:gstack-plan-design-review`
- User asks about developer experience of a plan, API/CLI/SDK design → invoke `/skill:gstack-plan-devex-review`
- User wants all reviews done automatically, "review everything" → invoke `/skill:gstack-autoplan`
- User reports a bug, error, broken behavior, "why is this broken", "this doesn't work", "wtf", "something's wrong" → invoke `/skill:gstack-investigate`
- User asks to test the site, find bugs, QA, "does this work", "check the deploy" → invoke `/skill:gstack-qa`
- User asks to just report bugs without fixing → invoke `/skill:gstack-qa-only`
- User asks to review code, check the diff, pre-landing review, "look at my changes" → invoke `/skill:gstack-review`
- User asks about visual polish, design audit of a live site, "this looks off" → invoke `/skill:gstack-design-review`
- User asks to audit the live developer experience, time-to-hello-world → invoke `/skill:gstack-devex-review`
- User asks to ship, deploy, push, create a PR, "let's land this", "send it" → invoke `/skill:gstack-ship`
- User asks to merge + deploy + verify as one flow → invoke `/skill:gstack-land-and-deploy`
- User asks to configure deployment for the project → invoke `/skill:gstack-setup-deploy`
- User asks to monitor prod after shipping, post-deploy checks → invoke `/skill:gstack-canary`
- User asks to update docs after shipping → invoke `/skill:gstack-document-release`
- User asks to write docs from scratch, generate documentation, "document this feature/module" → invoke `/skill:gstack-document-generate`
- User asks for a weekly retro, what did we ship, "how'd we do" → invoke `/skill:gstack-retro`
- User asks for a second opinion, codex review → invoke `/skill:gstack-codex`
- User asks for safety mode, careful mode → invoke `/skill:gstack-careful` or `/skill:gstack-guard`
- User asks to restrict edits to a directory → invoke `/skill:gstack-freeze` or `/skill:gstack-unfreeze`
- User asks to upgrade gstack → invoke `/skill:gstack-upgrade`
- User asks to save progress, checkpoint, "save my work" → invoke `/skill:gstack-context-save`
- User asks to resume, restore, "where was I" → invoke `/skill:gstack-context-restore`
- User asks about security, OWASP, vulnerabilities, "is this secure" → invoke `/skill:gstack-cso`
- User asks to make a PDF, document, publication → invoke `/skill:gstack-make-pdf`
- User asks to launch a real browser for QA, "open the browser" → invoke `/skill:gstack-open-gstack-browser`
- User asks to import cookies for authenticated testing → invoke `/skill:gstack-setup-browser-cookies`
- User asks about page speed, performance regression, benchmarks → invoke `/skill:gstack-benchmark`
- User asks what gstack has learned, "show learnings" → invoke `/skill:gstack-learn`
- User asks to tune question sensitivity, "stop asking me that" → invoke `/skill:gstack-plan-tune`
- User asks for code quality dashboard, "health check" → invoke `/skill:gstack-health`
