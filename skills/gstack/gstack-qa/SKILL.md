...
fix evidence, and a ship-readiness summary. For report-only mode, use /skill:gstack-qa-only.
...
C:/Users/Mattia/.pi/agent/extensions/gstack-pi/runtime/bin/gstack-timeline-log '{"skill":"qa","event":"started","branch":"'"$_BRANCH"'","session":"'"$_SESSION_ID"'"}' 2>/dev/null &
_HAS_ROUTING="no"
...
# /skill:gstack-qa: Test → Fix → Verify
...
"Your working tree has uncommitted changes. /skill:gstack-qa needs a clean tree so each bug fix gets its own atomic commit."
...
if [ "$_CROSS_PROJ" = "true" ]; then
  C:/Users/Mattia/.pi/agent/extensions/gstack-pi/runtime/bin/gstack-learnings-search --limit 10 --query "qa testing bug regression flake fixture" --cross-project 2>/dev/null || true
else
  C:/Users/Mattia/.pi/agent/extensions/gstack-pi/runtime/bin/gstack-learnings-search --limit 10 --query "qa testing bug regression flake fixture" 2>/dev/null || true
fi
```
...
```bash
C:/Users/Mattia/.pi/agent/extensions/gstack-pi/runtime/bin/gstack-learnings-search --query "<your-keyword>" --limit 5 2>/dev/null || true
```
...
- Include full attribution comment:
  ...
  // Found by /skill:gstack-qa on {YYYY-MM-DD}
...
2. **Fixed bugs that were in TODOS.md** → annotate with "Fixed by /skill:gstack-qa on {branch}, {date}"
...
```bash
C:/Users/Mattia/.pi/agent/extensions/gstack-pi/source/bin/gstack-learnings-log '{"skill":"qa","type":"TYPE","key":"SHORT_KEY","insight":"DESCRIPTION","confidence":N,"source":"SOURCE","files":["path/to/relevant/file"]}'
```
