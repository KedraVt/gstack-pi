...
description: |
  Codify the most recent successful /skill:gstack-scrape flow into a permanent browser-skill on disk. (gstack)
license: MIT
metadata:
  upstream: https://github.com/garrytan/gstack
  source: skillify/SKILL.md
---
# Pi adapter note
...
Future /skill:gstack-scrape calls with the same intent run
...
C:/Users/Mattia/.pi/agent/extensions/gstack-pi/runtime/bin/gstack-timeline-log '{"skill":"skillify","event":"started","branch":"'"$_BRANCH"'","session":"'"$_SESSION_ID"'"}' 2>/dev/null &
_HAS_ROUTING="no"
...
# /skill:gstack-skillify — codify the last scrape into a permanent skill

The productivity multiplier. `/skill:gstack-scrape` discovered how to pull the data;
`/skill:gstack-skillify` writes it as deterministic Playwright-via-`browse-client`
code so the next `/skill:gstack-scrape` call on the same intent runs in ~200ms.

Without this command, `/skill:gstack-scrape` is a slow wrapper around `$B`. With it,
...
for the most recent `/skill:gstack-scrape` invocation that:
...
> "No recent /skill:gstack-scrape result found in this conversation. Run /skill:gstack-scrape
> <intent> first, then say /skill:gstack-skillify."

Stop. Do not synthesize from chat fragments. Do not synthesize from a
match-path /skill:gstack-scrape result (matched skills are already codified — there's
nothing to skillify).
...
> "The last successful /skill:gstack-scrape was '<intent line>' a few turns back.
> Skillify that one?"
...
- 3–5 trigger phrases the agent should match against in future `/skill:gstack-scrape`
...
```
D<N> — Skill name + tier
Project/branch/task: codifying /skill:gstack-scrape "<intent>" as a browser-skill.
...
2. The active gstack skills install at `C:/Users/Mattia/.pi/agent/extensions/gstack-pi/source/`. Read
...
Use the helper at `C:/Users/Mattia/.pi/agent/extensions/gstack-pi/source/browse/src/browser-skill-write.ts`. Construct an inline
...
Project/branch/task: codified /skill:gstack-scrape "<intent>" — tests pass against fixture.
ELI10: The script ran clean against the snapshot we captured. Saying yes
moves the staged folder into ~/.gstack/browser-skills/ where /skill:gstack-scrape
...
/skill:gstack-scrape calls matching '<canonical-trigger>' will run in ~200ms."
...
- Codify match-path /skill:gstack-scrape results (matched skills are already codified)
- Codify mutating flows (those are /automate's job — Phase 2 P0)
- Run skills (that's `$B skill run` — codified skills are run via /skill:gstack-scrape's
...
```bash
C:/Users/Mattia/.pi/agent/extensions/gstack-pi/source/bin/gstack-learnings-log '{"skill":"skillify","type":"TYPE","key":"SHORT_KEY","insight":"DESCRIPTION","confidence":N,"source":"SOURCE","files":["path/to/relevant/file"]}'
```
