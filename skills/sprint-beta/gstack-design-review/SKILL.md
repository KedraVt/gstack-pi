...
screenshots. For plan-mode design review (before implementation), use /skill:gstack-plan-design-review.
...
C:/Users/Mattia/.pi/agent/extensions/gstack-pi/runtime/bin/gstack-timeline-log '{"skill":"design-review","event":"started","branch":"'"$_BRANCH"'","session":"'"$_SESSION_ID"'"}' 2>/dev/null &
_HAS_ROUTING="no"
...
# /skill:gstack-design-review: Design Audit → Fix → Verify
...
"Your working tree has uncommitted changes. /skill:gstack-design-review needs a clean tree so each design fix gets its own atomic commit."
...
$B url
```
If URL contains `/login`, `/signin`, `/auth`, or `/sso`: the site requires authentication. AskUserQuestion: "This site requires authentication. Want to import cookies from your browser? Run `/skill:gstack-setup-browser-cookies` first if needed."
...
Use the same scorecard format as /skill:gstack-plan-design-review (shown above). Fill in from both outputs.
Merge findings into the triage with `[codex]` / `[subagent]` / `[cross-model]` tags.

**Log the result:**
```bash
C:/Users/Mattia/.pi/agent/extensions/gstack-pi/runtime/bin/gstack-review-log '{"skill":"design-outside-voices","timestamp":"'"$(date -u +%Y-%m-%dT%H:%M:%SZ)"'","status":"STATUS","source":"SOURCE","commit":"'"$(git rev-parse --short HEAD)"'"}'
```
...
For CSS-only fixes: skip entirely. CSS regressions are caught by re-running /skill:gstack-design-review.

If the fix involved JS behavior: follow the same procedure as /skill:gstack-qa Phase 8e.5 (study existing
...
2. **Fixed findings that were in TODOS.md** → annotate with "Fixed by /skill:gstack-design-review on {branch}, {date}"
...
```bash
C:/Users/Mattia/.pi/agent/extensions/gstack-pi/source/bin/gstack-learnings-log '{"skill":"design-review","type":"TYPE","key":"SHORT_KEY","insight":"DESCRIPTION","confidence":N,"source":"SOURCE","files":["path/to/relevant/file"]}'
```
