...
C:/Users/Mattia/.pi/agent/extensions/gstack-pi/runtime/bin/gstack-timeline-log '{"skill":"document-release","event":"started","branch":"'"$_BRANCH"'","session":"'"$_SESSION_ID"'"}' 2>/dev/null &
_HAS_ROUTING="no"
...
You are running the `/skill:gstack-document-release` workflow. This runs **after `/skill:gstack-ship`** (code committed, PR
...
- **Coverage map informs, never generates.** The Diataxis coverage map flags gaps for the PR body
  ...
  are found, suggest `/skill:gstack-document-generate` as the follow-up skill.
