...
C:/Users/Mattia/.pi/agent/extensions/gstack-pi/runtime/bin/gstack-timeline-log '{"skill":"ship","event":"started","branch":"'"$_BRANCH"'","session":"'"$_SESSION_ID"'"}' 2>/dev/null &
_HAS_ROUTING="no"
...
> Let gstack proactively suggest skills, like /skill:gstack-qa for "does this work?" or /skill:gstack-investigate for bugs?
...
> Tip: gstack pays off when you complete one loop — **plan → review → ship**. A common first loop: `/skill:gstack-office-hours` or `/skill:gstack-spec` to shape it, `/skill:gstack-plan-eng-review` to lock it, then `/skill:gstack-ship`.
...
Key routing rules:
- Product ideas/brainstorming → invoke /skill:gstack-office-hours
- Strategy/scope → invoke /skill:gstack-plan-ceo-review
- Architecture → invoke /skill:gstack-plan-eng-review
- Design system/plan review → invoke /skill:gstack-design-consultation or /skill:gstack-plan-design-review
- Full review pipeline → invoke /skill:gstack-autoplan
- Bugs/errors → invoke /skill:gstack-investigate
- QA/testing site behavior → invoke /skill:gstack-qa or /skill:gstack-qa-only
- Code review/diff check → invoke /skill:gstack-review
- Visual polish → invoke /skill:gstack-design-review
- Ship/deploy/PR → invoke /skill:gstack-ship or /skill:gstack-land-and-deploy
- Save progress → invoke /skill:gstack-context-save
- Resume context → invoke /skill:gstack-context-restore
- Author a backlog-ready spec/issue → invoke /skill:gstack-spec
```
...
if [ -f "$_GBRAIN_CONFIG" ] && command -v gbrain >/dev/null 2>&1; then
  ...
  if [ "$_GBRAIN_VERSION_OK" -gt 0 ] 2>/dev/null; then
    ...
    if [ -n "$_GBRAIN_PIN_PATH" ]; then
      ...
      echo "Run /skill:gstack-sync-gbrain to refresh."
    else
      echo "GBrain configured but this worktree isn't pinned yet. Run \`/skill:gstack-sync-gbrain --full\`"
...
# Detect remote-MCP mode (Path 4 of /skill:gstack-setup-gbrain). Local artifacts sync is
...
safety, and /skill:gstack-ship review gates. If a nudge below conflicts with skill instructions,
...
```bash
eval "$(C:/Users/Mattia/.pi/agent/extensions/gstack-pi/runtime/bin/gstack-slug 2>/dev/null)"
...
if [ -d "$_PROJ" ]; then
  ...
  if [ -f "$_PROJ/decisions.active.json" ]; then
    ...
    C:/Users/Mattia/.pi/agent/extensions/gstack-pi/source/bin/gstack-decision-search --recent 5 2>/dev/null
    echo "--- END DECISIONS ---"
  fi
  echo "--- END ARTIFACTS ---"
fi
...
**Cross-session decisions.** If `ACTIVE DECISIONS` are listed, treat them as prior settled calls with their rationale — do not silently re-litigate them; if you're about to reverse one, say so explicitly. Reach for `C:/Users/Mattia/.pi/agent/extensions/gstack-pi/source/bin/gstack-decision-search` whenever a question touches a past decision ("what did we decide / why / did we try"). When you or the user make a DURABLE decision (architecture, scope, tool/vendor choice, or a reversal) — NOT a turn-level or trivial choice — log it with `C:/Users/Mattia/.pi/agent/extensions/gstack-pi/source/bin/gstack-decision-log` (`--supersede <id>` for a reversal). Reliable and local; gbrain not required.
...
Curated jargon list lives at `C:/Users/Mattia/.pi/agent/extensions/gstack-pi/source/scripts/jargon-list.json` (80+ terms). On the first jargon term you encounter this session, Read that file once; treat the `terms` array as the canonical list. The list is repo-owned and may grow between releases.
...
`/skill:gstack-context-restore` reads `[gstack-context]`; `/skill:gstack-ship` squashes WIP commits into clean commits.
...
If you are looping on the same diagnostic, same file, or failed fix variants, STOP and reassess. Consider escalation or /skill:gstack-context-save. Progress summaries must NEVER mutate git state.

## Question Tuning (skip entirely if `QUESTION_TUNING: false`)

Before each AskUserQuestion, choose `question_id` from `scripts/question-registry.ts` or `{skill}-{slug}`, then run `C:/Users/Mattia/.pi/agent/extensions/gstack-pi/runtime/bin/gstack-question-preference --check "<id>"`. `AUTO_DECIDE` means choose the recommended option and say "Auto-decided [summary] → [option] (your preference). Change with /skill:gstack-plan-tune." `ASK_NORMALLY` means ask.
...
```bash
C:/Users/Mattia/.pi/agent/extensions/gstack-pi/runtime/bin/gstack-question-preference --write '{"question_id":"<id>","preference":"<pref>","source":"inline-user","free_text":"<optional original words>"}'
```
...
Before building anything unfamiliar, **search first.** See `C:/Users/Mattia/.pi/agent/extensions/gstack-pi/source/ETHOS.md`.
...
Skills that run plan reviews (`/plan-*-review`, `/skill:gstack-codex review`) include the EXIT PLAN MODE GATE blocking checklist at the end of the skill, which verifies the plan file ends with `## GSTACK REVIEW REPORT` before ExitPlanMode is called. Skills that don't run plan reviews (operational skills like `/skill:gstack-ship`, `/skill:gstack-qa`, `/skill:gstack-review`) typically don't operate in plan mode and have no review report to verify; this footer is a no-op for them. Writing the plan file is the one edit allowed in plan mode.
...
You are running the `/skill:gstack-ship` workflow. This is a **non-interactive, fully automated** workflow. Do NOT ask for confirmation at any step. The user said `/skill:gstack-ship` which means DO IT. Run straight through and output the PR URL at the end.
...
Re-running `/skill:gstack-ship` means "run the whole checklist again." Every verification step
...
Never skip a verification step because a prior `/skill:gstack-ship` run already performed it.
...
```bash
C:/Users/Mattia/.pi/agent/extensions/gstack-pi/runtime/bin/gstack-review-read
```

Parse the output. Find the most recent entry for each skill (plan-ceo-review, plan-eng-review, review, plan-design-review, design-review-lite, adversarial-review, codex-review, codex-plan-review). Ignore entries with timestamps older than 7 days. For the Eng Review row, show whichever is more recent between `review` (diff-scoped pre-landing review) and `plan-eng-review` (plan-stage architecture review). Append "(DIFF)" or "(PLAN)" to the status to distinguish. For the Adversarial row, show whichever is more recent between `adversarial-review` (new auto-scaled) and `codex-review` (legacy). For Design Review, show whichever is more recent between `plan-design-review` (full visual audit) and `design-review-lite` (code-level check). Append "(FULL)" or "(LITE)" to the status to distinguish. For the Outside Voice row, show the most recent `codex-plan-review` entry — this captures outside voices from both /skill:gstack-plan-ceo-review and /skill:gstack-plan-eng-review.

**Source attribution:** If the most recent entry for a skill has a \`"via"\` field, append it to the status label in parentheses. Examples: `plan-eng-review` with `via:"autoplan"` shows as "CLEAR (PLAN via /skill:gstack-autoplan)". `review` with `via:"ship"` shows as "CLEAR (DIFF via /skill:gstack-ship)". Entries without a `via` field show as "CLEAR (PLAN)" or "CLEAR (DIFF)" as before.
...
- **Outside Voice (optional):** Independent plan review from a different AI model. Offered after all review sections complete in /skill:gstack-plan-ceo-review and /skill:gstack-plan-eng-review. Falls back to Claude subagent if Codex is unavailable. Never gates shipping.
...
Check diff size: `git diff <base>...HEAD --stat | tail -1`. If the diff is >200 lines, add: "Note: This is a large diff. Consider running `/skill:gstack-plan-eng-review` or `/skill:gstack-autoplan` for architecture-level review before shipping."

If CEO Review is missing, mention as informational ("CEO Review not run — recommended for product changes") but do NOT block.

For Design Review: run `source <(C:/Users/Mattia/.pi/agent/extensions/gstack-pi/runtime/bin/gstack-diff-scope <base> 2>/dev/null)`. If `SCOPE_FRONTEND=true` and no design review (plan-design-review or design-review-lite) exists in the dashboard, mention: "Design Review not run — this PR changes frontend code. The lite design check will run automatically in Step 9, but consider running /skill:gstack-design-review for a full visual audit post-implementation." Still never block.
...
> **STOP.** Before running the test suites and (if prompt files changed) the eval suites (Steps 4-6), Read `C:/Users/Mattia/.pi/agent/extensions/gstack-pi/source/ship/sections/tests.md` and execute it
> in full. Do not work from memory — that section is the source of truth for this step.

> **STOP.** Before auditing test coverage of the diff (Step 7), Read `C:/Users/Mattia/.pi/agent/extensions/gstack-pi/source/ship/sections/test-coverage.md` and execute it
> in full. Do not work from memory — that section is the source of truth for this step.

> **STOP.** Before auditing plan completion, verification, and scope drift (Step 8), Read `C:/Users/Mattia/.pi/agent/extensions/gstack-pi/source/ship/sections/plan-completion.md` and execute it
> in full. Do not work from memory — that section is the source of truth for this step.

> **STOP.** Before the pre-landing review and specialist dispatch (Step 9), Read `C:/Users/Mattia/.pi/agent/extensions/gstack-pi/source/ship/sections/review-army.md` and execute it
> in full. Do not work from memory — that section is the source of truth for this step.

> **STOP.** Before addressing Greptile review comments when a PR exists (Step 10), Read `C:/Users/Mattia/.pi/agent/extensions/gstack-pi/source/ship/sections/greptile.md` and execute it
> in full. Do not work from memory — that section is the source of truth for this step.

> **STOP.** Before the adversarial review and learnings capture (Step 11), Read `C:/Users/Mattia/.pi/agent/extensions/gstack-pi/source/ship/sections/adversarial.md` and execute it
> in full. Do not work from memory — that section is the source of truth for this step.
...
1. **Classify state** — pure reader, never writes:
   ```bash
   bun run C:/Users/Mattia/.pi/agent/extensions/gstack-pi/runtime/bin/gstack-version-bump classify --base <base>
   ```
   ...
   - **DRIFT_UNEXPECTED** → **STOP**. package.json disagrees with VERSION while VERSION matches base — a manual edit bypassed /skill:gstack-ship. Reconcile manually, then re-run.
...
3. **Queue-aware pick** (workspace-aware ship):
   ```bash
   QUEUE_JSON=$(bun run C:/Users/Mattia/.pi/agent/extensions/gstack-pi/runtime/bin/gstack-next-version --base <base> --bump "$BUMP_LEVEL" --current-version "$BASE_VERSION" 2>/dev/null || echo '{"offline":true}')
...
4. **Write the bump** (FRESH, or an approved rebump):
   ```bash
   bun run C:/Users/Mattia/.pi/agent/extensions/gstack-pi/runtime/bin/gstack-version-bump write --version "$NEW_VERSION"
   ```
...
5. **Record the release decision** (durable cross-session memory). The bump level is a real decision the next session should not re-derive blind:
   ```bash
   C:/Users/Mattia/.pi/agent/extensions/gstack-pi/source/bin/gstack-decision-log '{"decision":"Ship NEW_VERSION (BUMP_LEVEL)","rationale":"WHY","scope":"repo","source":"skill","confidence":9}' 2>/dev/null || true
   ```
...
> **STOP.** Before writing the CHANGELOG entry (Step 13), Read `C:/Users/Mattia/.pi/agent/extensions/gstack-pi/source/ship/sections/changelog.md` and execute it
> in full. Do not work from memory — that section is the source of truth for this step.
...
Read `C:/Users/Mattia/.pi/agent/extensions/gstack-pi/source/review/TODOS-format.md` for the canonical format reference.
...
```bash
_REDACT_PREPUSH=$(C:/Users/Mattia/.pi/agent/extensions/gstack-pi/runtime/bin/gstack-config get redact_prepush_hook 2>/dev/null || echo "false")
...
1. **`REDACT_PREPUSH: true` and `HOOK_INSTALLED: no` and `HOOKS_IN_GIT_DIR: yes`** —
   ...
   ```bash
   C:/Users/Mattia/.pi/agent/extensions/gstack-pi/source/bin/gstack-redact install-prepush-hook
   ```
...
2. **`REDACT_PREPUSH` not true AND `PREPUSH_PROMPTED: no`** — one-time
   ...
   If A: run `C:/Users/Mattia/.pi/agent/extensions/gstack-pi/runtime/bin/gstack-config set redact_prepush_hook true`
   then `C:/Users/Mattia/.pi/agent/extensions/gstack-pi/source/bin/gstack-redact install-prepush-hook`.
   If B: run `C:/Users/Mattia/.pi/agent/extensions/gstack-pi/runtime/bin/gstack-config set redact_prepush_hook false`.
...
**PR/MR title invariant (always applies — do not skip even if you don't open the section below):** Any PR or MR you create OR update in the next step MUST have a title that starts with `v$NEW_VERSION` (the version bumped in Step 12), in the format `v<NEW_VERSION> <type>: <summary>`. Never create or edit a PR/MR title without this prefix. Compute the correct title with the single source of truth helper: `C:/Users/Mattia/.pi/agent/extensions/gstack-pi/runtime/bin/gstack-pr-title-rewrite.sh "$NEW_VERSION" "<current title>"`. The full create/update procedure (idempotency, redaction scan, self-check) is in the section below.

> **STOP.** Before syncing docs and creating or updating the PR/MR (Steps 18-19), Read `C:/Users/Mattia/.pi/agent/extensions/gstack-pi/source/ship/sections/pr-body.md` and execute it
> in full. Do not work from memory — that section is the source of truth for this step.

## Step 20: Persist ship metrics

Log coverage and plan completion data so `/skill:gstack-retro` can track trends:

```bash
eval "$(C:/Users/Mattia/.pi/agent/extensions/gstack-pi/runtime/bin/gstack-slug 2>/dev/null)" && mkdir -p ~/.gstack/projects/$SLUG
```
...
Plan-tune cathedral T15. After a successful ship, surface /skill:gstack-plan-tune once
per machine. Single line, non-blocking, marker-gated so it never re-fires.

```bash
_NUDGE_MARKER="$HOME/.gstack/.plan-tune-nudge-shown"
_QT=$(C:/Users/Mattia/.pi/agent/extensions/gstack-pi/runtime/bin/gstack-config get question_tuning 2>/dev/null || echo "false")
if [ ! -f "$_NUDGE_MARKER" ] && [ "$_QT" = "false" ]; then
  echo ""
  echo "gstack can learn from your AskUserQuestion answers. Run /skill:gstack-plan-tune to opt in"
...
- **The goal is: user says `/skill:gstack-ship`, next thing they see is the review + PR URL + auto-synced docs.**
