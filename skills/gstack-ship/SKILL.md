...
name: gstack-ship
description: |
  Ship workflow: detect + merge base branch, run tests, review diff, bump VERSION, update
  CHANGELOG, commit, push, create PR. (gstack)
license: MIT
metadata:
...
C:/Users/Mattia/.pi/agent/gstack-pi/repo/bin/gstack-timeline-log '{"skill":"ship","event":"started","branch":"'"$_BRANCH"'","session":"'"$_SESSION_ID"'"}' 2>/dev/null &
_HAS_ROUTING="no"
...
```bash
eval "$(C:/Users/Mattia/.pi/agent/gstack-pi/repo/bin/gstack-slug 2>/dev/null)"
...
if [ -d "$_PROJ" ]; then
  ...
  if [ -f "$_PROJ/decisions.active.json" ]; then
    ...
    C:/Users/Mattia/.pi/agent/gstack-pi/repo/bin/gstack-decision-search --recent 5 2>/dev/null
    echo "--- END DECISIONS ---"
  fi
  echo "--- END ARTIFACTS ---"
fi
...
Curated jargon list lives at `C:/Users/Mattia/.pi/agent/gstack-pi/repo/scripts/jargon-list.json` (80+ terms). On the first jargon term you encounter this session, Read that file once; treat the `terms` array as the canonical list. The list is repo-owned and may grow between releases.
...
Before each AskUserQuestion, choose `question_id` from `scripts/question-registry.ts` or `{skill}-{slug}`, then run `C:/Users/Mattia/.pi/agent/gstack-pi/repo/bin/gstack-question-preference --check "<id>"`. `AUTO_DECIDE` means choose the recommended option and say "Auto-decided [summary] → [option] (your preference). Change with /skill:gstack-plan-tune." `ASK_NORMALLY` means ask.
...
```bash
C:/Users/Mattia/.pi/agent/gstack-pi/repo/bin/gstack-question-log '{"skill":"ship","question_id":"<id>","question_summary":"<short>","category":"<approval|clarification|routing|cherry-pick|feedback-loop>","door_type":"<one-way|two-way>","options_count":N,"user_choice":"<key>","recommended":"<key>","session_id":"'"$_SESSION_ID"'"}' 2>/dev/null || true
```
...
```bash
C:/Users/Mattia/.pi/agent/gstack-pi/repo/bin/gstack-question-preference --write '{"question_id":"<id>","preference":"<pref>","source":"inline-user","free_text":"<optional original words>"}'
```
...
Before building anything unfamiliar, **search first.** See `C:/Users/Mattia/.pi/agent/gstack-pi/repo/ETHOS.md`.
...
```bash
C:/Users/Mattia/.pi/agent/gstack-pi/repo/bin/gstack-review-read
```
...
For Design Review: run `source <(C:/Users/Mattia/.pi/agent/gstack-pi/repo/bin/gstack-diff-scope <base> 2>/dev/null)`. If `SCOPE_FRONTEND=true` and no design review (plan-design-review or design-review-lite) exists in the dashboard, mention: "Design Review not run — this PR changes frontend code. The lite design check will run automatically in Step 9, but consider running /skill:gstack-design-review for a full visual audit post-implementation." Still never block.
...
> **STOP.** Before running the test suites and (if prompt files changed) the eval suites (Steps 4-6), Read `C:/Users/Mattia/.pi/agent/gstack-pi/repo/ship/sections/tests.md` and execute it
> in full. Do not work from memory — that section is the source of truth for this step.

> **STOP.** Before auditing test coverage of the diff (Step 7), Read `C:/Users/Mattia/.pi/agent/gstack-pi/repo/ship/sections/test-coverage.md` and execute it
> in full. Do not work from memory — that section is the source of truth for this step.

> **STOP.** Before auditing plan completion, verification, and scope drift (Step 8), Read `C:/Users/Mattia/.pi/agent/gstack-pi/repo/ship/sections/plan-completion.md` and execute it
> in full. Do not work from memory — that section is the source of truth for this step.

> **STOP.** Before the pre-landing review and specialist dispatch (Step 9), Read `C:/Users/Mattia/.pi/agent/gstack-pi/repo/ship/sections/review-army.md` and execute it
> in full. Do not work from memory — that section is the source of truth for this step.

> **STOP.** Before addressing Greptile review comments when a PR exists (Step 10), Read `C:/Users/Mattia/.pi/agent/gstack-pi/repo/ship/sections/greptile.md` and execute it
> in full. Do not work from memory — that section is the source of truth for this step.

> **STOP.** Before the adversarial review and learnings capture (Step 11), Read `C:/Users/Mattia/.pi/agent/gstack-pi/repo/ship/sections/adversarial.md` and execute it
> in full. Do not work from memory — that section is the source of truth for this step.
...
1. **Classify state** — pure reader, never writes:
   ```bash
   bun run C:/Users/Mattia/.pi/agent/gstack-pi/repo/bin/gstack-version-bump classify --base <base>
   ```
...
3. **Queue-aware pick** (workspace-aware ship):
   ```bash
   QUEUE_JSON=$(bun run C:/Users/Mattia/.pi/agent/gstack-pi/repo/bin/gstack-next-version --base <base> --bump "$BUMP_LEVEL" --current-version "$BASE_VERSION" 2>/dev/null || echo '{"offline":true}')
...
4. **Write the bump** (FRESH, or an approved rebump):
   ```bash
   bun run C:/Users/Mattia/.pi/agent/gstack-pi/repo/bin/gstack-version-bump write --version "$NEW_VERSION"
   ```
...
5. **Record the release decision** (durable cross-session memory). The bump level is a real decision the next session should not re-derive blind:
   ```bash
   C:/Users/Mattia/.pi/agent/gstack-pi/repo/bin/gstack-decision-log '{"decision":"Ship NEW_VERSION (BUMP_LEVEL)","rationale":"WHY","scope":"repo","source":"skill","confidence":9}' 2>/dev/null || true
   ```
...
> **STOP.** Before writing the CHANGELOG entry (Step 13), Read `C:/Users/Mattia/.pi/agent/gstack-pi/repo/ship/sections/changelog.md` and execute it
> in full. Do not work from memory — that section is the source of truth for this step.
...
Read `C:/Users/Mattia/.pi/agent/gstack-pi/repo/C:/Users/Mattia/.pi/agent/gstack-pi/repo/review/TODOS-format.md` for the canonical format reference.
...
```bash
_REDACT_PREPUSH=$(C:/Users/Mattia/.pi/agent/gstack-pi/repo/bin/gstack-config get redact_prepush_hook 2>/dev/null || echo "false")
...
1. **`REDACT_PREPUSH: true` and `HOOK_INSTALLED: no` and `HOOKS_IN_GIT_DIR: yes`** —
   ...
   ```bash
   C:/Users/Mattia/.pi/agent/gstack-pi/repo/bin/gstack-redact install-prepush-hook
   ```
...
2. **`REDACT_PREPUSH` not true AND `PREPUSH_PROMPTED: no`** — one-time
   ...
   If A: run `C:/Users/Mattia/.pi/agent/gstack-pi/repo/bin/gstack-config set redact_prepush_hook true`
   then `C:/Users/Mattia/.pi/agent/gstack-pi/repo/bin/gstack-redact install-prepush-hook`.
   If B: run `C:/Users/Mattia/.pi/agent/gstack-pi/repo/bin/gstack-config set redact_prepush_hook false`.
...
```bash
eval "$(C:/Users/Mattia/.pi/agent/gstack-pi/repo/bin/gstack-slug 2>/dev/null)" && mkdir -p ~/.gstack/projects/$SLUG
```
...
_QT=$(C:/Users/Mattia/.pi/agent/gstack-pi/repo/bin/gstack-config get question_tuning 2>/dev/null || echo "false")
