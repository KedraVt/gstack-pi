...
const gstackRoot = resolve(here, "..", "..", "gstack");
...
function main() {
  const srcSkillsDir = join(gstackRoot, ".pi", "skills");
  const dstSkillsDir = join(gstackPiRoot, "skills");

  if (!existsSync(srcSkillsDir)) {
    console.error(`Error: Source skills dir missing at ${srcSkillsDir}. Run \`bun run gen:skill-docs:user\` in gstack/ first.`);
    process.exit(1);
  }
